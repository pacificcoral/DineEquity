...
class RowDD$1 {
    ...
    updateIcon(row, index, args) {
        ...
        let rowPositionHeight = rowEle.offsetTop - scrollTop;
...
let TreeGrid = TreeGrid_1 = class TreeGrid extends Component {
    ...
    localExpand(action, row, record, isChild) {
        ...
        let rows = gridRows.filter((r) => r.classList.contains('e-gridrowindex' + record.index + 'level' + (record.level + 1)));
        let movableRows = this.getMovableRows().filter((r) => r.classList.contains('e-gridrowindex' + record.index + 'level' + (record.level + 1)));
        for (let i = 0; i < rows.length; i++) {
